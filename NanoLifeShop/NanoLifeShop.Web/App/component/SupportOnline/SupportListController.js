(
    function (app) {
        app.controller('SupportListController', SupportListController);

        SupportListController.$inject = ['$scope', 'apiService', 'notificationService', '$ngBootbox', '$filter'];
        function SupportListController($scope, apiService, notificationService, $ngBootbox, $filter) {
            ...
            function GetlistData(page) {
                ...
                apiService.get("/api/supportonline/getall", config, function (response) {

                    if (response.data.TotalCount == 0) {
                        notificationService.DisplayWarning("Không có dữ liệu !");
                    }
                    $scope.DataSupport = response.data.Items;
            ...
            function SelecteAll() {
                if ($scope.isSelectedAll === false) {
                    angular.forEach($scope.DataSupport, function (item) {
                        item.check = true;
                    });
            ...
            $scope.$watch("DataSupport", function (n, o) {
            ...
            function DeleteItem(id) {
                ...
                $ngBootbox.confirm('Bạn muốn xóa danh sách này ?')
                    .then(function () {
                        var config = {
                            params: {
                                ID: id
                        ...
                        apiService.dele('/api/supportonline/delete', config, function (result) {
            ...
            function DeleMultiProCate() {
                ...
                $ngBootbox.confirm('Bạn muốn xóa những danh mục này ?')
                    .then(function () {
                        ...
                        apiService.dele('/api/supportonline/deleteMulti', config, function (response) {
